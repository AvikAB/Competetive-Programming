...
vector<ll> leftView(Node* root){   // level order traversal
    ...
    while(!q.empty()){
        ...
        while(lvlsz--){   // then the other ele are not so important, just check their children - thats why remove them
            Node* temp = q.front();
            q.pop();
...
void Lview(Node* root, ll level, vector<ll>&ans){
    if(!root) return;
