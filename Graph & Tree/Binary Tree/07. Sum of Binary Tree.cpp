...
void totSum(Node* root, ll &sum){
    if(root==NULL) return;

    sum += (root->data);        // taking sum of current node
    totSum(root->left, sum);    // visit left
    totSum(root->right, sum);   // visit right
}
...
int totSum(Node* root){
    ...
    return root->data + totSum(root->left) + totSum(root->right);
}
